...
const CONTENT_PATTERNS = [
    ...
    },
    {
        ...
        regex: /\b(?:postgres(?:ql)?|mongodb(?:\+srv)?|mysql|redis):\/\/[^\s'"`<>]*:[^\s'"`<>@]+@[^\s'"`<>]+/,
    },
    {
        name: "hardcoded password",
        regex: /(?:^|[^A-Za-z0-9_])(?:password|passwd|pwd)\s*[:=]\s*['"][^'"\s]{6,}['"]/i,
    },
];
export function scanFilenames(files) {
    const findings = [];
...
export function scanSecrets(files, diff) {
    return {
